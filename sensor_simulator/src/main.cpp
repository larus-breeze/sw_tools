...
int main(int, char**){
    std::cout << "Hello, from sensor_simulator!\n";
...
uint32_t system_state // fake system state here in lack of hardware
    = GNSS_AVAILABLE | MTI_SENSOR_AVAILABE | MS5611_STATIC_AVAILABLE |
...
int main(int argc, char *argv[]) {
  ...
  // don't enable UNDERFLOW as this can happen regularly when filter outputs
  // decay
  feenableexcept(FE_DIVBYZERO | FE_INVALID | FE_OVERFLOW);
#endif

  if ((argc != 2) && (argc != 3)) {
    printf("usage: %s infile.f37 [skiptime for TCP version]\n", argv[0]);
    return -1;
  }
  ...
  if (realtime_with_TCP_server) {
    ...
    realtime_with_TCP_server = accept_TCP_client(false);
    realtime_with_TCP_server = true; //Test
  }
  ...
  if ((dot != 0) &&
      (dot[1] == 'f')) // old format: filename.f37.EEPROM new: filename.EEPROM
  ...
  auto until =
      awake_time(std::chrono::steady_clock::now()); // start with now + 100ms
  ...
  for (count = 1; count < records; ++count) {
    ...
    if (have_GNSS_fix == false) {
      if (output_data[count].c.sat_fix_type > 0) {
        organizer.update_magnetic_induction_data(
            output_data[count].c.latitude, output_data[count].c.longitude);
    ...
    if (output_data[count].c.nano != nano) // 10 Hz by GNSS
    {
      ...
      counter_10Hz = 1; // synchronize the 10Hz processing as early as new data
                        // are observed
    }
    ...
    if (count % 10 == 0) {
      if (realtime_with_TCP_server) {
        ...
        if (count % 40 == 0)
          format_NMEA_string_fast((const output_data_t &)*(output_data + count),
                                  buffer, true);

        if (count % 160 == 0)
          format_NMEA_string_slow((const output_data_t &)*(output_data + count),
                                  buffer);
...
void report_magnetic_calibration_has_changed(
    magnetic_induction_report_t *p_magnetic_induction_report, char) {
  magnetic_induction_report_t magnetic_induction_report =
      *p_magnetic_induction_report;
  char buffer[50];
  ...
  for (unsigned i = 0; i < 3; ++i) {
    ...
    *next++ = ' ';
    next =
        my_ftoa(next, SQRT(magnetic_induction_report.calibration[i].variance));
    *next++ = ' ';
    *next++ = 0;
    printf("%s\t", buffer);
  }
