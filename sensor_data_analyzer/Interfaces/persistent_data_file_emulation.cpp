...
bool read_meta_data_file (char *file_path)
{
  ...
  if (config_file.is_open ())
      ...
      if (size != (EEPROM_FILE_SYSTEM_SIZE * sizeof(uint32_t)))
	{
      ...
      if ( not permanent_data_file.file_is_consistent ())
	{
      ...
      if (read_EEPROM_file (path) != EXIT_SUCCESS)
	{
      ...
      result = temp_file.store_data (HORIZON, round (value));
      assert(result == true);

      value = configuration (GNSS_CONFIGURATION);
      result = temp_file.store_data (GNSS_CONFIGURATION,
					       round (value));
      assert(result == true);
      ...
      // do some tests
      assert( permanent_data_file.file_is_consistent());
      permanent_data_file.dump_all_entries ();
